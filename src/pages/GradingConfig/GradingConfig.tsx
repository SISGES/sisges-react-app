...
const inputCls =
  "px-3 py-2 text-sm w-full bg-[var(--color-input-bg)] border border-[var(--color-border)] rounded-md text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/10 disabled:opacity-50 disabled:cursor-not-allowed transition-colors";
const inputErrCls =
  "border-[var(--color-error)] focus:border-[var(--color-error)] focus:ring-[var(--color-error)]/10";
const labelCls =
  "text-xs font-medium text-[var(--color-text-muted)] mb-1 block";
...
export function GradingConfig() {
  ...
  const [trimesterOpen, setTrimesterOpen] = useState<
  ...
  const trimesterSum = useMemo(
    () => (t1Max ?? 0) + (t2Max ?? 0) + (t3Max ?? 0),
    [t1Max, t2Max, t3Max],
  );
  const sumValid = yearMaxPoints !== null && trimesterSum === yearMaxPoints;
  const yearValid =
    yearMaxPoints !== null &&
    yearMaxPoints >= YEAR_MIN &&
    yearMaxPoints <= YEAR_MAX;
  ...
  const compositionByTrimester = useMemo(() => {
    const row = (
      ...
      const p = parseIntOrNull(provas),
        a = parseIntOrNull(ativ),
        tr = parseIntOrNull(trab);
  ...
  const handlePctBlur = (s: string, setField: (v: string) => void) => {
    ...
    const c = Math.min(100, Math.max(0, n));
    setField(
      Number.isInteger(c) ? String(c) : String(Math.round(c * 100) / 100),
    );
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const ym = parseIntOrNull(yearMaxStr),
      yp = parsePctOrNull(yearMinStr);
    const p1 = parsePctOrNull(t1MinStr),
      p2 = parsePctOrNull(t2MinStr),
      p3 = parsePctOrNull(t3MinStr);
    const t1p = parseIntOrNull(t1ProvasStr),
      t1a = parseIntOrNull(t1AtividadesStr),
      t1t = parseIntOrNull(t1TrabalhosStr);
    const t2p = parseIntOrNull(t2ProvasStr),
      t2a = parseIntOrNull(t2AtividadesStr),
      t2t = parseIntOrNull(t2TrabalhosStr);
    const t3p = parseIntOrNull(t3ProvasStr),
      t3a = parseIntOrNull(t3AtividadesStr),
      t3t = parseIntOrNull(t3TrabalhosStr);
    if (
      [
        ym,
        yp,
          ...
          >
            ...
            <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-5 flex flex-col gap-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label htmlFor="yearMax" className={labelCls}>
                    Pontuação máxima
                  </label>
                  <input
                    id="yearMax"
                    ...
                    onChange={(e) =>
                      setYearMaxStr(
                        normalizePointsInput(e.target.value, YEAR_MAX),
                      )
                    }
                  ...
                  {yearMaxStr !== "" && !yearValid && (
                    <p className="text-xs text-[var(--color-error)] mt-1">
                      Entre {YEAR_MIN} e {YEAR_MAX}
                    </p>
                  )}
                </div>
                <div>
                  <label htmlFor="yearMin" className={labelCls}>
                    Média mínima (%)
                  </label>
            ...
            {trimesterData.map((t) => {
              ...
              const hasTrimesterIssue = !(
                tm !== null &&
                tm >= 1 &&
                parsePctOrNull(t.minStr) !== null &&
                comp.valid
              );
                  ...
                  >
                    ...
                    {hasTrimesterIssue && (
                      <FiAlertCircle
                        ...
                        title="Campos em falta ou inválidos"
                      />
                    )}
                  </button>
                  ...
                  {isOpen && (
                    <div className="px-5 pb-5 flex flex-col gap-4 border-t border-[var(--color-border)]">
                      <div className="mt-4 grid gap-4 md:grid-cols-2">
                        <div>
                          <label htmlFor={`t${idx}Max`} className={labelCls}>
                            Pontuação máxima
                          </label>
                          <input
                            id={`t${idx}Max`}
                        ...
                        <div>
                          <label htmlFor={`t${idx}Pct`} className={labelCls}>
                            Média mínima (%)
                          </label>
                          <div className="relative">
                            <input
                              ...
                              onBlur={() =>
                                handlePctBlur(t.minStr, t.setMinStr)
                              }
                              className={`${inputCls} pr-8`}
                              disabled={isSaving}
                            />
                      ...
                      <div className="flex flex-col gap-2">
                        <p className="text-xs font-medium text-[var(--color-text-muted)]">
                          Provas, atividades e trabalhos (pontos)
                        </p>
                        <div className="grid gap-3 md:grid-cols-3">
                          {[
                            ...
                            <div key={f.id}>
                              <label htmlFor={f.id} className={labelCls}>
                                {f.label}
                              </label>
                              <input
                                id={f.id}
                                ...
                                onChange={(e) =>
                                  f.set(
                                    normalizeCompositionPointsInput(
                                      e.target.value,
                                      TRIMESTER_POINTS_MAX,
                                    ),
                                  )
            ...
            >
              <span>
                Soma dos trimestres: {trimesterSum} / {yearMaxPoints ?? "—"}
              </span>
              {sumValid ? (
                <span>OK</span>
              ) : (
                <span>A soma deve ser igual à pontuação do ano</span>
              )}
            </div>
