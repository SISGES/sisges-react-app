      ...
      try {
        ...
        if (!cancelled) {
          ...
          (data.students ?? []).forEach((s) => {
            initial[s.id] = s.present === false ? "F" : "P";
          });
  ...
  if (!aulaId || isNaN(aulaId)) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <p className="text-sm text-[var(--color-error)]">ID inválido.</p>
      </div>
    );
  }
    ...
    <div className="flex items-center gap-2">
      {canEdit && (
        <button
          ...
          title="Editar"
          className="flex items-center justify-center w-8 h-8 rounded-md border border-[var(--color-border)] bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] cursor-pointer transition-colors"
        >
          <FiEdit2 size={15} />
        </button>
      )}
      {isAdmin && (
        <button
          ...
          title="Excluir"
          disabled={isDeleting}
          className="flex items-center justify-center w-8 h-8 rounded-md border border-[var(--color-border)] bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-error)] hover:border-[var(--color-error)] cursor-pointer transition-colors disabled:opacity-50"
        >
          <FiTrash2 size={15} />
        </button>
      )}
                ...
                <div className="mt-4 border-t border-[var(--color-border)] pt-4 flex items-center justify-between gap-3">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="flex items-center justify-center w-7 h-7 rounded-md border border-[var(--color-border)] bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] cursor-pointer transition-colors"
                    >
                      <FiInfo size={14} />
                    </button>
                  )}
                            ...
                            <div className="flex flex-col gap-0.5">
                              ...
                              {a.filePath && (
                                <ProtectedDownloadButton
                                  path={a.filePath}
                                  className="cursor-pointer border-none bg-transparent p-0 text-xs text-[var(--color-primary)] hover:underline"
                                >
                                  Baixar documento
                                </ProtectedDownloadButton>
                              )}
                      ...
                      >
                        ...
                        {isAdmin && (
                          <button
                            ...
                            title="Ver detalhes"
                            className="flex items-center justify-center w-7 h-7 rounded-md border border-[var(--color-border)] bg-transparent text-[var(--color-text-muted)] hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] cursor-pointer transition-colors flex-shrink-0"
                          >
                            <FiInfo size={14} />
                          </button>
                        )}
