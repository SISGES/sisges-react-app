...
const selectCls =
  "w-full px-3 py-2 text-sm bg-[var(--color-input-bg)] border border-[var(--color-border)] rounded-md text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors";
const inputCls =
  "w-full px-3 py-2 text-sm bg-[var(--color-input-bg)] border border-[var(--color-border)] rounded-md text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-primary)] disabled:opacity-50 transition-colors";
...
export function CreateAula() {
  ...
  const [disciplineOptions, setDisciplineOptions] = useState<
    TeacherDisciplineOption[]
  >([]);
  const [schoolClassOptions, setSchoolClassOptions] = useState<
    ...
    try {
      ...
      for (const c of classDetails) {
        ...
        for (const d of c.disciplines) {
          ...
          options.push({
            discipline: d,
            schoolClassIds: disciplineMap.get(d.id) ?? [],
          });
        }
      }
      ...
      setOptionsError(
        err instanceof ApiError ? err.message : "Erro ao carregar opções.",
      );
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        (existingAulas ?? []).some((a) =>
          timesOverlap(startTime, endTime, a.startTime ?? "", a.endTime ?? ""),
        )
      ) {
        setSubmitError(
          "Já existe uma aula cadastrada nesta turma no mesmo dia com horário sobreposto.",
        );
          ...
          <StateBlock
            ...
            error={
              ...
              (disciplineOptions.length === 0
                ? "Você não possui disciplinas ou turmas vinculadas."
                : null)
            }
            ...
            >
                  ...
                  <label className="flex items-center gap-2 text-sm text-[var(--color-text-primary)] cursor-pointer whitespace-nowrap">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setDate(e.target.checked ? getTodayBr() : "");
                      }}
                      disabled={isSubmitting}
              ...
              <div className="flex gap-3 justify-end">
                ...
                <Button
                  ...
                  disabled={!isTimeValid || !isDateValid}
                >
                  Criar Aula
                </Button>
              </div>
