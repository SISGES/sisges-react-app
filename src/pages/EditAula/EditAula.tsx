...
const inputCls =
  "w-full px-3 py-2 text-sm bg-[var(--color-input-bg)] border border-[var(--color-border)] rounded-md text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)] disabled:opacity-50 transition-colors";
...
export function EditAula() {
  ...
  const fetchAula = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Erro ao carregar aula.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    (async () => {
      try {
        ...
        for (let i = 0; i < classDetails.length; i++) {
          ...
          if (c?.disciplines?.some((d) => d.id === disciplineId))
            validClassIds.push(classIds[i]);
        }
        const classes = await Promise.all(
          validClassIds.map((cid) =>
            getClassById(cid).then((cc) => ({ id: cc.id, name: cc.name })),
          ),
        );
        setSchoolClasses(classes);
        setSchoolClassId((prev) => {
          const validIds = classes.map((c) => c.id);
          return validIds.includes(prev as number)
            ? prev
            : (classes[0]?.id ?? "");
        });
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        ...
        setSubmitError(
          "Já existe uma aula nesta turma no mesmo dia com horário sobreposto.",
        );
      ...
      setSubmitError(
        err instanceof ApiError ? err.message : "Erro ao atualizar aula.",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleSubmitFrequency = async () => {
    ...
    try {
      ...
      setFreqError(
        err instanceof ApiError ? err.message : "Erro ao lançar frequência.",
      );
    } finally {
          ...
          footer={
            <>
              <Button
                ...
                disabled={isSubmittingFreq}
              >
                Cancelar
              </Button>
              <Button
                onClick={handleSubmitFrequency}
                loading={isSubmittingFreq}
              >
                Salvar
              </Button>
            </>
                  ...
                  {(["P", "F"] as const).map((status) => (
                    ...
                    >
                      <input
                        type="radio"
                        name={`freq-${s.id}`}
                        checked={attendance[s.name] === status}
                        onChange={() =>
                          setAttendance((p) => ({ ...p, [s.name]: status }))
                        }
