    ...
    try {
      if (editing) {
        ...
        await createDiscipline({
          ...
          description: description.trim() || undefined,
          teacherIds:
            selectedTeacherIds.length > 0 ? selectedTeacherIds : undefined,
        });
      ...
      setSubmitError(
        err instanceof ApiError ? err.message : "Erro ao salvar disciplina.",
      );
    } finally {
                ...
                <tbody>
                  {disciplines.map((d) => (
                    <tr key={d.id} className={tableStyles.trHover}>
                      ...
                      <td
                        className={`${tableStyles.td} text-[var(--color-text-muted)] max-w-xs truncate`}
                      >
                        {d.description || "—"}
                      </td>
