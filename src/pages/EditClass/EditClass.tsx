...
export function EditClass() {
  ...
  const [schoolClass, setSchoolClass] = useState<ClassDetailResponse | null>(
    null,
  );
  ...
  const [showCreateDisciplineModal, setShowCreateDisciplineModal] =
    useState(false);
  ...
  const fetchClass = useCallback(async () => {
    ...
    setError(null);
    try {
      setSchoolClass(await getClassById(classId));
    } catch (err) {
      setError(
  ...
  const handleConfirmUnlink = async () => {
    ...
    setActionLoading(
      type === "student"
        ? `remove-student-${person.id}`
        : `remove-teacher-${person.id}`,
    );
    setConfirmModal(null);
    try {
      setSchoolClass(
        await (type === "student"
          ? removeStudentFromClass(classId, person.id)
  ...
  const handleAdd = async (
    action: () => Promise<ClassDetailResponse>,
    key: string,
  ) => {
  ...
  const confirmMsg = confirmModal ? (
    confirmModal.type === "discipline" ? (
      <span>
        Remover a disciplina <strong>{confirmModal.discipline.name}</strong> da
        turma?
      </span>
    ) : (
      <span>
        Desvincular{" "}
        {confirmModal.type === "student" ? "o aluno" : "o professor"}{" "}
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
          ...
          {schoolClass && (
            <div className="flex flex-col gap-6">
              <SectionPanel
                ...
                onRemove={(s) =>
                  setConfirmModal({ type: "student", person: s })
                }
                ...
                <AddRow
                  ...
                  onAdd={() => {
                    ...
                    if (!isNaN(studentId))
                      handleAdd(
                        () =>
                          addStudentToClass(classId, studentId).then((r) => {
              ...
              <SectionPanel
                ...
                onRemove={(t) =>
                  setConfirmModal({ type: "teacher", person: t })
                }
                actionLoading={actionLoading}
                ...
                <AddRow
                  ...
                  onAdd={() => {
                    ...
                    if (!isNaN(teacherId))
                      handleAdd(
                        () =>
                          addTeacherToClass(classId, teacherId).then((r) => {
                  ...
                  <AddRow
                    ...
                    onAdd={() => {
                      ...
                      if (!isNaN(disciplineId))
                        handleAdd(
                          () =>
                            addDisciplineToClass(classId, disciplineId).then(
                              (r) => {
      ...
      <ConfirmModal
        ...
        message={confirmMsg ?? ""}
        confirmLabel={
          confirmModal?.type === "discipline" ? "Remover" : "Desvincular"
        }
      />
          ...
          <ul className="flex flex-col divide-y divide-[var(--color-border)]">
            {items.map((item) => (
              <li
                ...
                className="flex items-center justify-between gap-3 py-2.5"
              >
                <div className="text-sm text-[var(--color-text-primary)] flex-1 min-w-0">
                  {renderItem(item)}
                </div>
                <UnlinkBtn
