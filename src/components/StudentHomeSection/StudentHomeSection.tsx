...
const thClass =
  "text-left text-xs font-semibold uppercase tracking-wide text-[var(--color-text-muted)] pb-2 border-b border-[var(--color-border)]";
const tdClass =
  "py-2 border-b border-[var(--color-border)] text-[var(--color-text-primary)] last:border-0";
...
export function StudentHomeSection({
  ...
  const [absencesByDiscipline, setAbsencesByDiscipline] = useState<
    DisciplineAbsenceRow[]
  >([]);
    ...
    <div className="content-reveal flex flex-col gap-4">
      ...
      {showClass && (
        <SectionCard title={variant !== "class" ? "Minha turma" : undefined}>
          {!myClass?.className ? (
            <p className="text-sm text-[var(--color-text-muted)]">
              Nenhuma turma vinculada à sua conta.
            </p>
          ) : (
            <div className="flex flex-col gap-5">
              ...
              <div>
                ...
                {myClass.classmates.length === 0 ? (
                  <p className="text-sm text-[var(--color-text-muted)]">
                    Não há outros alunos nesta turma.
                  </p>
                ) : (
      ...
      {showAbsences && (
        <SectionCard
          title={variant !== "absences" ? "Faltas por disciplina" : undefined}
        >
          ...
          {!myClass?.className ? (
            <p className="text-sm text-[var(--color-text-muted)]">
              Disponível quando você estiver vinculado a uma turma.
            </p>
          ) : absencesByDiscipline.length === 0 ? (
            <p className="text-sm text-[var(--color-text-muted)]">
              Nenhuma disciplina na turma.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <table className={tableClass}>
